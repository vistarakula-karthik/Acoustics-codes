...
data = load("shock region.txt");
f = data(:,1)';
PSD_exp = data(:,2);        %.*2292.4;
...
disp(" 4 --> for Seperated flow region")
disp(" ")
...
elseif n==4
    ...
    disp("  ")
    x_sp = 0.914;
    Rexsp = (u_infty*x_sp)/nu;
    delta_star_sp = (0.0371/(Rexsp)^(0.2)*((9/7 + 0.475*M^2)/(1 + 0.13*M^2)^0.64))*x_sp;
    ff_sp = (2*pi*f*delta_star_sp)/u_infty;
    Prms_q_sp = min(0.026,(0.041/(1 + 1.606*M^2)));
    G=(4*(Prms_q_sp)^2*((F)^(1.433)./(1 + F^(2.867)*(ff_sp).^2)))*(delta_star_sp*q^2/u_infty);                    
...
%% 
 ...
 p_ref = 20e-6;
 Cprms = 0.07;       %0.010917                  % This is in the attached flow region on the hammerhead vehicle(43.2 in)
 Prms = Cprms*q; 
 SPL_exp = 20*log10(Prms/p_ref)
%% 

 final_OASPL = [OASPL SPL_exp];
 T1 = array2table(final_OASPL, 'VariableNames', {'OASPL_code','OASPL_exp'});
 disp(T1);
