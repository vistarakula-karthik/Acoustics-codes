...
import pandas as pd
# %%
import inputs_trial as it

# 1. Call the function (the popup appears)
data = it.get_params()

# 2. Extract variables (This replaces your hardcoded list)
if data:
    n, F, ue, me = data['number of engines'], data['Thrust from each engine(N)'], data['exit velocity(m/s)'], data['exit mach number']
    dei, x2, angle = data['exit diameter'], data['impingement point'], data['uplift angle']
    eta = data['Efficiency']
    L = data['Basestand length(m)']
    W = data['Basestand width(m)']
    H = data['Basestand height(m)']
...
# --------------------------------------------------------
#            2D microphone arrangement
# --------------------------------------------------------
'''
x_vals = np.arange(-1000, 1001, 100)          # (start stop step)
y_vals = np.arange(-1000, 1001, 100)


X_cord, Y_cord = np.meshgrid(x_vals, y_vals)

mic_locations = np.column_stack((
    X_cord.ravel(),
    Y_cord.ravel(),
    np.zeros(X_cord.size)
))
'''
nozzle_location = np.array([0, 0, 0])


Woa = eta*0.005*n*F*ue;
...
delta_f = f_up - f_lr
delta_f = delta_f.reshape(1, -1)  # Convert to 1 row, 101 columns


#print(f"{'F_lr (Hz)':<12} {'F_center (Hz)':<12} {'F_up (Hz)':<12}")

#for ff, s, o in zip(f_lr, f_up, f):
    #print(f"{ff:<12.3f} {s:<12.3f} {o:<12.3f}")
...
# Axial position (x1/xt)
x = np.arange(0, 5, 0.05)   # 0 to 5 with step 0.05

# Make sure xt and Lw are defined before this
delta_x = 0.05 * xt

# Normalised Acoustic Power Per Unit Core Length
NAPPUCL = (-0.1167 * x**6
           + 1.7081 * x**5
           - 9.6015 * x**4
           + 26.812 * x**3
...
NAPPUCL = NAPPUCL.reshape(1, -1)  # Convert to 1 row, 101 columns
# Overall acoustic power for each slice
...
# Plot actual data points as black diamonds
ax.scatter(deg, strn, di, color='black', marker='d', s=10, depthshade=False, label='Experimental Data')

# Labels and Styling
ax.set_xlabel('Angle (deg)')
ax.set_ylabel('Strouhal Number')
ax.set_zlabel('Directivity Index (dB)')
ax.set_title('Python: Smoothed Thin Plate Spline Directivity Map')

# Set camera angle to match your screenshot
ax.view_init(elev=20, azim=-125)

# Clip Z-axis to keep view focused on relevant data
ax.set_zlim(di.min() - 5, di.max() + 5)
...
'''
r = 39
angles = np.array([20, 35, 50, 65, 80])
rad = np.radians(angles)

# Create the mic_location array
# Logic: [r*cos(theta), 0, -r*sin(theta)]
mic_location = np.array([
    [r * np.cos(rad[0]), 0, -r * np.sin(rad[0])],
    [r * np.cos(rad[1]), 0, -r * np.sin(rad[1])],
    [r * np.cos(rad[2]), 0, -r * np.sin(rad[2])],
    [r * np.cos(rad[3]), 0, -r * np.sin(rad[3])],
    [r * np.cos(rad[4]), 0, -r * np.sin(rad[4])]
]) 

mic_location = -np.fliplr(mic_location)
'''          
mic_location = np.array([
    [0.033, 0, 0.33],
    [0.033, 0, 0.66],
...
for x1 in range(5):
    ...
    for k in range(1, 100):
        ...
        # Determine source location X
        X1 = k * 0.05 * xt
        X_mag = np.linalg.norm(X1)
        ...
        # Compute Lwsb
        aa = 10 * np.log10(delta_f)

        bb = 10 * np.log10(ue * ao / (X_mag * ae))

        Lwsb = (
            npsl_array
            + 10 * np.log10(delta_f)
            + Lws[0, k]
            - 10 * np.log10(ue * ao / (X_mag * ae))
        )
...
# 2. Scale the coordinates
        scaled_coords = scaler.transform(input_coords)

# 3. Call the rbf_smooth function using the scaled columns
# We use scaled_coords[:, 0] for Angle and scaled_coords[:, 1] for Strouhal
        SPLsbp = Lwsb - 10 * np.log10(4 * np.pi * r2) + rbf_smooth(scaled_coords[:, 0], scaled_coords[:, 1])
        #SPLsbp = Lwsb - 10 * np.log10(4 * np.pi * r2) + rbf_smooth(*scaler.transform([[Theta, stn]])[0])

        toSPL = toSPL + (10 ** (SPLsbp / 10))

    SPLbp[:, x1] = 10 * np.log10(toSPL)
    
    SPLbp_average = np.zeros((SPLbp.shape[0], 2))

    SPLbp_average[:,0] = np.mean(SPLbp, axis = 1)

    totSPLbp = np.sum(10 ** (SPLbp[:, x1] / 10))

    SPLOA_1 = 10 * np.log10(totSPLbp)

    SPLOA.append(SPLOA_1)
   
df = pd.DataFrame(mic_location, columns=['X (m)', 'Y (m)', 'Z (m)'])

# 2. Add the OASPL values as the fourth column
# Make sure OASPL_values is a list or array of length 5
df['OASPL (dB)'] = SPLOA

# 3. Display the table
print("--- Microphone Coordinates and Acoustic Results ---")
print(df)
# %%
...
# 1. Set font to Times New Roman (or closest serif match)
plt.rcParams["font.family"] = "serif"
plt.rcParams["font.serif"] = ["Times New Roman"] + plt.rcParams["font.serif"]

# Create the plot
fig, ax = plt.subplots(figsize=(7, 7))  # Square figure size

for spine in ax.spines.values():
    spine.set_linewidth(2.0)  # Increase border thickness to 2.0
# Define a list of symbols to cycle through
symbols = ['o', 's', '^', 'v', 'D', '<', '>']

# 2. Loop and plot each microphone
num_mics = SPLbp_average.shape[1]
for i in range(num_mics):
    ax.plot(f_signal, SPLbp_average[:, i], 
            label=f'Mic {i+1}', 
            marker=symbols[i % len(symbols)], 
            markersize=5,
            markevery=0.1, # Shows symbols at 10% intervals so it's not too crowded
            linewidth=2)

# 3. Configure Axis Scales and Aspect Ratio
ax.set_xscale('log')
ax.set_box_aspect(1)  # Forces the plot area to be a perfect square

# 4. Turn on Minor Ticks and Grids
ax.minorticks_on()

ax.tick_params(axis='both', which='major', labelsize=11, 
               width=2.0, length=8, direction='in', top=True, right=True)
# Minor Ticks
ax.tick_params(axis='both', which='minor', 
               width=1.5, length=4, direction='in', top=True, right=True)
# Major grid
ax.grid(which='major', linestyle='-', linewidth='0.5', color='black', alpha=0.7)
# Minor grid
ax.grid(which='minor', linestyle=':', linewidth='1', color='gray', alpha=0.5)

ax.set_xlim(20, 10000) 

# 2. Set Y-axis limits (Sound Pressure Level)
# Assuming your values fall between 40 and 140 dB
# You can also use np.min(SPLbp) and np.max(SPLbp) to automate this
ax.set_ylim(80, 140)

# 5. Labels and Title with LaTeX
ax.set_xlabel('Frequency ($Hz$)', fontsize=12, fontname="Times New Roman")
ax.set_ylabel('Sound Pressure Level ($dB$)', fontsize=12, fontname="Times New Roman")
ax.set_title('Acoustic SPL Spectrum', fontsize=14, fontweight='bold', fontname="Times New Roman")
...
# 6. Save the output
plt.tight_layout()
plt.savefig('acoustic_spectrum_square.png', dpi=300)
# %%
import pandas as pd

# 1. Take your existing 'df' (Coordinates and OASPL)
# Add an empty column to act as the "Gap"
df[' '] = "" 

# 2. Create a dictionary for the Spectral Data
spectral_dict = {'Frequency (Hz)': f_signal}

for i in range(SPLbp.shape[1]):
    spectral_dict[f'Mic {i+1} SPL (dB)'] = SPLbp[:, i]

# 3. Convert to DataFrame
df_spectral = pd.DataFrame(spectral_dict)

# 4. Concatenate side-by-side
df_final = pd.concat([df, df_spectral], axis=1)

# --- NEW STEP: ROUND ALL NUMERICAL DATA TO 3 DECIMALS ---
df_final = df_final.round(3)

# 5. Save to CSV
df_final.to_csv("Acoustic_Full_Data_Report.csv", index=False)

# 6. Display a preview
print("--- Final Combined Table Preview (Rounded to 3 Decimals) ---")
...
# (Replace 'your_file_name.csv' with your actual CSV file)
# Using header=None assumes your CSV is just raw numbers. If it has headers, remove header=None
csv_filename = 'jato_undeflected_data.csv'
ref_df = pd.read_csv(csv_filename, header=None)

# Extract frequency (column 0) and SPL (column 1) from the CSV
f_csv = ref_df.iloc[:, 0].to_numpy()
spl_csv = ref_df.iloc[:, 1].to_numpy()
# ------------------------------
...
# Define a list of symbols to cycle through for the estimated data
symbols = ['o', 's', '^', 'v', 'D', '<', '>']

# 2. Loop and plot each microphone's ESTIMATED data (from your code)
num_mics = SPLbp_average.shape[1]
#for i in range(num_mics):
ax.plot(f_signal, SPLbp_average[:,0],
        label=f'Empirical (code)',
        marker=symbols[i % len(symbols)],
        markersize=5,
        markevery=0.1, # Shows symbols at 10% intervals
        linewidth=2)

# 3. Plot the CSV DATA on the SAME graph
# Plotted with a dashed red line and a star marker so it stands out
ax.plot(f_csv, spl_csv,
        label='Experimental',
        color='red',
        linestyle='--',
        marker='*',
...
# 4. Configure Axis Scales and Aspect Ratio
ax.set_xscale('log')
ax.set_box_aspect(1)  # Forces the plot area to be a perfect square

# 5. Turn on Minor Ticks and Grids
ax.minorticks_on()
...
# 1. Take your existing 'df' (Coordinates and OASPL)
df[' '] = "" # Add an empty column to act as a "Gap"

# 2. Create a dictionary for the Spectral Data
spectral_dict = {'Frequency (Hz)': f_signal}

for i in range(SPLbp_average.shape[1]):
    spectral_dict[f'Mic {i+1} SPL (dB)'] = SPLbp_average[:, i]
...
# 5. Round all numerical data to 3 decimals and save
df_final = df_final.round(3)
df_final.to_csv("Acoustic_Full_Data_Report.csv", index=False)
